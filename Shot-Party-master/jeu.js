...
var multiplicateur = 1; //redimensionnement des objets en fonction de la taille de l'�cran
...
var score_equipe = [0,0];
var temps_restant = 2000;
...
function afficherScore() {
	fill(0);
	textSize(35);
	text(score_equipe[0] + " : " + score_equipe[1],740*multiplicateur/2,595*multiplicateur);
}

function deconte() {
	if(temps_restant>=0){
		temps_restant-=1
		text(Math.ceil(temps_restant/100),740*multiplicateur/4,595*multiplicateur);
	}else {
		fill(150,0,0);
		textSize(70);
		if(score_equipe[0]<score_equipe[1]){
			text("equipe rouge gagne",740*multiplicateur/4,250*multiplicateur);
		}else if(score_equipe[0]=score_equipe[1]){
			text("egaliteS",740*multiplicateur/4,250*multiplicateur);
		}else if(score_equipe[0]>score_equipe[1]){
			text("equipe bleu gagne",740*multiplicateur/4,250*multiplicateur);
		}
		text("GAME OVER",740*multiplicateur/4,250*multiplicateur);
	}
...
{	
  ...
  if(keyIsDown(32)) {
	...
	if (tirer[0] == true)	{
		...
		if (temps_balle[0] == 50*multiplicateur) {
			tirer[0] = false;
			temps_balle[0] = 0;
			}
	}
...
function tirJoueur2()
{
	...
	if(tirer[1] == true)
	{
		...
		if(temps_balle[1] == 50*multiplicateur){
			tirer[1] = false;
			temps_balle[1] = 0;
		}
	}
